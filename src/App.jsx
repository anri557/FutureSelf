...
const EMAILJS_SERVICE_ID = "service_49dp7sl";
const EMAILJS_TEMPLATE_ID = "template_tyb911m";
const EMAILJS_PUBLIC_KEY = "f2F4qHJS2edoYMpoW";
  ...
  const sealMessage = async () => {
    ...
    emailjs.send(
      ...
      EMAILJS_TEMPLATE_ID,
      {
        ...
        to_email: email,
        message: body,
        sent_date: new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" }),
        open_date: new Date(openDate).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" }),
      },
      EMAILJS_PUBLIC_KEY
    ).then(() => {
      showToast("✦ Letter sealed & email sent!");
    }).catch(() => {
      showToast("✦ Letter saved but email failed");
    });
